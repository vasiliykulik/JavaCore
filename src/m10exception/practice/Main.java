...
public class Main {
    public static void main(String[] args) {
       ...
       /* Number number = 0.1;
        ...
        try {
            //  number = Integer.parseInt("ABC"); // number format // не приведется

            result = integer / (Integer) number; // <- class cast // Пытается авто боксинг к double - не получается - Ошибка Cast  - Противная ошибка
...
//      number = array[6]; // array index out of bounds
      ...
      *//*
      очень часто возникают проблемы - система пытается взять пустой List (после сбоя - раз в месяц)
       *//*
        } catch (Exception e) {
        ...
        try {
            String string = null;
            String test;
            string.toUpperCase(); // IDEA ловит что может быть NullPointerException - Объект во многих местах может быть null,
            // try и catch делать на NullPointer  делать нельзя - работать if -- try catch - достаточно массивный.
            // После try b catch код как и после if тоже пойдет дальше
...
//    requestParams2.put("email", "mail@example.com"); // обязательный
        ...
        try {
            ...
            String saveResult = controller1.save(requestParams1);
            System.out.println(saveResult);

           // String findResult = controller1.findByEmail(requestParams2);
